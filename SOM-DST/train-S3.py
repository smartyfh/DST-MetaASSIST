...
import higher
import itertools
from WeightNet import WNet2 as WNet 
...
def get_unreduced_loss_and_feat(state_scores, op2id, slot_meta, tokenizer, op_ids, gen_scores, gen_ids, 
        ...
        for epoch in trange(int(args.n_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader)):
                ...
                with higher.innerloop_ctx(meta_model, meta_optimizer) as (meta_m, meta_opt):
                    ...
                    loss_slot_gt, loss_slot_pseudo, loss_feat = get_unreduced_loss_and_feat(state_scores, op2id, 
                    slot_meta, tokenizer, op_ids, gen_scores, gen_ids, op_ids_pseudo, gen_scores_pseudo, gen_ids_pseudo)
                    
                    weight_gt, weight_pseudo = L2WNet(loss_feat.detach())
                    weight_gt = weight_gt.squeeze(2)
                    weight_pseudo = weight_pseudo.squeeze(2)
                    weight = torch.sum(weight_gt + weight_pseudo, 0)
                    weight[weight==0.0] = 1.0
                    
                    meta_loss = torch.sum(torch.div(weight_gt*loss_slot_gt + weight_pseudo*loss_slot_pseudo, weight))
                    # first backward
                ...
                loss = torch.sum(torch.div(weight_gt*loss_slot_gt + weight_pseudo*loss_slot_pseudo, weight))
                # backward (base model)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", default='output-meta24-S3/exp', type=str)
    ...
    parser.add_argument("--wnet_lr", default=1e-5, type=float)
