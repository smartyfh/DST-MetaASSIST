...
MIT license
The code has been modified by us
"""
...
class SomDST(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids,
                ...
                max_value_pseudo=None, op_ids_pseudo=None, max_update_pseudo=None, teacher_pseudo=None):
        ...
        gen_scores = self.decoder(input_ids, decoder_inputs, sequence_output, pooled_output, max_value, 
                                  self.encoder.bert.embeddings.word_embeddings, teacher)

        if use_pseudo:
            # need to generate gen_scores for the pseudo labels
            gathered_pseudo = []
            for b, a in zip(state_output, op_ids_pseudo.eq(self.update_id)):  # update
                if a.sum().item() != 0:
                    v = b.masked_select(a.unsqueeze(-1)).view(1, -1, self.hidden_size)
                    n = v.size(1)
                    ...
                    v = torch.zeros(1, max_update_pseudo, self.hidden_size, device=input_ids.device)
                gathered_pseudo.append(v)
            decoder_inputs_pseudo = torch.cat(gathered_pseudo) # batch, max_update, hidden
            ###########
            
            gen_scores_pseudo = self.decoder(input_ids, decoder_inputs_pseudo, sequence_output,
                                             pooled_output, max_value_pseudo, 
                                             self.encoder.bert.embeddings.word_embeddings, teacher_pseudo)

            return state_scores, gen_scores, gen_scores_pseudo
...
class Encoder(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids,
        ...
        state_output = torch.gather(sequence_output, 1, state_pos)  # B, J, 768
...
#         self.embed.weight = deepcopy(bert_model_embedding_weights)
    ...
    def forward(self, x, decoder_input, encoder_output, hidden, max_len, embed, teacher=None):
        ...
        for j in range(n_update):
            ...
            for k in range(max_len):
                ...
                # B,D * D,V => B,V
                attn_v = torch.matmul(hidden.squeeze(0), embed.weight.transpose(0, 1))  # B,V
...
#                 slot_value.append([ww.tolist() for ww in w_idx])
                if teacher is not None:
                    w = embed(teacher[:, j, k]).unsqueeze(1)
                else:
                    w = embed(w_idx).unsqueeze(1)  # B,1,D
